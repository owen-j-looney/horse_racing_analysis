...
This analysis devles into horse racing data, and attempting to identify ways to predict which horse will win, and the best way to make a profit from betting on horse racing in Australia. 
...
The horse racing data initially looks like this:

```{r data download and clean, echo=FALSE}
...
After scraping the data, it was decided to convert each horses run into an approximate time estimation, by taking each race's winning time, and the using the trailing horses distance and estimating their final time by adding time allowing for each horse to cover their losing distance at the winners average speed throughout the race. These times look like this:
...
cat(glue("The GLM's largest prediction for a horse winning is {round(max(fitted.results)*100,2)}%"))
```

As the GLM found no way to make any predictions where a horse would win, it was decided that we will try using a random forest to predict some winning horses via classification. 
...
cat(glue("The Random forest does not predict a lot of horses to win (only {sum(predTrain==1)}), however its accuracy of {round(mean(predTrain == test_horse_data$winner)*100,2)}% is quite impressive."))
...
For the above, we can see that the neural net does predict more horses to win than the random forest, however its prediction of which horses will win is lower than the random forest. When selecting a model to pick a winning horse, a random forest would be used with the chosen variables used. 

## Making money

It has occurred to me that since favourites win approximately 40% of the time, it would be entirely possible to double down on betting each day until a favourite wins to generate a specified income daily - for lack of a better term, essentially making gambling on horses a side hustle.
...
#calculating the bet amount for each individual race, and the total outlay, for races after an initial race has failed. so all bet amount would be taken off the total_outlay once this is complete
...
cat("below is a table showing all races bet on during the month, and the profit of each race")
